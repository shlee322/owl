...
public class MongoDB {
	...
	public boolean Add_User(String Id, String Pw) {
		try {
	...
	public ArrayList<User> Load_UserList() {
	...
	public boolean Update_User(String Id, String NewPw) {
		try {
			UserColl = UserDB.getCollection("User");
			BasicDBObject doc;
	...
	public boolean Del_User(String Id, String Pw) {
		try {
			m.dropDatabase(Id);
			UserColl = UserDB.getCollection("User");
			BasicDBObject doc;
	...
	// 그룹 추가
	public boolean Add_Group(String G_Name) {
		try {
			GroupName = G_Name;
	...
	// 사람 추가
	public boolean Add_Person(String G_Name, String P_Name,
	...
	public ArrayList<Person> Load_GroupList(String G_Name) {
	...
	// 그룹
	public boolean Update_Group(String GroupName, String Re_GroupName) {
		try {
	...
	// 그룹멤버
	public boolean Update_Person(String GroupName, ArrayList<Person> P1) {
		try {
			GroupColl = AdressDB.getCollection(GroupName);

			for (Person person : P1) {
				BasicDBObject doc;
				doc = MakePersonDocument(person.Name, person.Mail_Address,
						person.Phone);
				GroupColl.update(new BasicDBObject().append("_id",
						person.ObjectID), doc);
			}
			return true;
		} catch (Exception e) {
	...
	public boolean Del_Group(String G_Name) {
		try {
			GroupColl = AdressDB.getCollection(G_Name);
			GroupColl.drop();
	...
	// 메일 내용부분 추가
	public boolean Add_Mail_Content(Boolean Sending, long Send_Time,
			int Send_Num, String From_Adress, String Mail_Title,
			String Mail_Content, String UserName) {
		try {
			SendMailColl = SendMailDB.getCollection("Mail_Content");

			SendMailColl.insert(MakeSendMailDocument(Sending, Send_Time,
					Send_Num, From_Adress, Mail_Title, Mail_Content, UserName));
	...
	// 받는 사람 정보 추가
	public boolean Add_To_Person(long Send_Time, String To_Adress,
			String Group_Name) {
		try {
			...
			SendMailColl.insert(MakeToPersonDocument(Sending, Check_Time,
					To_Adress, Cord, Group_Name, Key));
	...
	public ArrayList<Send_Mail> Load_Mail_List(long day, long checkday) {
		DBCursor cur;
		SendMailColl = SendMailDB.getCollection("Mail_Content");
		...
		if (checkday == 0) {
			cur = SendMailColl.find(new BasicDBObject().append("time",
					new BasicDBObject("$lte", day)));// <=
			return Add_Mail_List(cur);
		}
		// 이상
		else if (checkday == 1) {
			cur = SendMailColl.find(new BasicDBObject().append("time",
					new BasicDBObject("$gte", day)));// >=
			return Add_Mail_List(cur);
		}
		// 사이값
		else {
	...
	public ArrayList<Send_Mail> Add_Mail_List(DBCursor cur) {
		...
		while (cur.hasNext()) {
			...
			Mail_Temp.Send_Num = Integer.parseInt(cur.curr().get("num")
					.toString());
			Mail_Temp.Send_Time = Long.parseLong(cur.curr().get("time")
					.toString());
	...
	public ArrayList<To_Sender_Person> Load_Sender_Person(Long Time, int num) {
		...
		if (Personcur.count() != 0) {
			for (int i = 0; i < num; i++) {
				...
				Temp_Person.To_Adress = Personcur.curr().get("toaddress")
						.toString();
				Temp_Person.Key = Integer.parseInt(Personcur.curr().get("key")
						.toString());
		...
		for (To_Sender_Person person : Sender_Person_List) {
			BasicDBObject newDocument3 = new BasicDBObject().append("$set",
					new BasicDBObject().append("sending", True));
			ObjectId id = new ObjectId(person.ObjectID);
			SendMailColl.update(new BasicDBObject().append("_id", id),
					newDocument3);
		}
		return Sender_Person_List;
	}

	public boolean Update_MailList(Send_Mail send) {
		try {
			SendMailColl = AdressDB.getCollection("Mail_Content");
			BasicDBObject doc;
			doc = MakeSendMailDocument(send.Sending, send.Send_Time,
					send.Send_Num, send.From_Adress, send.Mail_Title,
					send.Mail_Content, send.UserName);
			SendMailColl.update(new BasicDBObject().append("time",
					send.Send_Time), doc);
			return true;
		} catch (Exception e) {
			return false;
	...
	public boolean Update_MailList_User(ArrayList<To_Person> person,
			String SendTime) {
		try {
			...
			for (To_Person Person : person) {
				doc = MakeToPersonDocument(Person.Sending, Person.Check_Time,
						Person.To_Adress, Person.Cord, Person.Group_Name,
						Person.Key);
				ObjectId id = new ObjectId(Person.ObjectID);
	...
	public boolean Del_MailList(Send_Mail send) {
		try {
			SendMailColl = SendMailDB.getCollection("Mail_Content");
			SendMailColl.remove(new BasicDBObject().append("time",
					send.Send_Time));
			SendMailColl = SendMailDB.getCollection(Long
					.toString(send.Send_Time));
			SendMailColl.drop();
			return true;
		} catch (Exception e) {
			return false;
	...
	public boolean Update_Cord(long Send_Time, String ObjectID, String Cord) {
		try {
			SendMailColl = SendMailDB.getCollection(Long.toString(Send_Time));
			ObjectId id = new ObjectId(ObjectID);

			BasicDBObject newDocument3 = new BasicDBObject().append("$set",
					new BasicDBObject().append("cord", Cord));
			SendMailColl.update(new BasicDBObject().append("_id", id),
					newDocument3);
	...
	public boolean Update_CheckTime(long Send_Time, String ObjectID, int key) {
		try {
			long Check_Time;
			...
			BasicDBObject newDocument3 = new BasicDBObject().append("$set",
					new BasicDBObject().append("checktime", Check_Time));
			SendMailColl.update(new BasicDBObject().append("_id", id).append(
					"key", key), newDocument3);
	...
	private static BasicDBObject MakePersonDocument(String P_Name,
			String Mail_Address, String Phone) {
		BasicDBObject doc = new BasicDBObject();
		doc.put("name", P_Name);
	...
	private static BasicDBObject MakeSendMailDocument(Boolean Sending,
			long Send_Time, int Send_Num, String From_Adress,
			String Mail_Title, String Mail_Content, String UserName) {
		BasicDBObject doc = new BasicDBObject();
	...
	private static BasicDBObject MakeToPersonDocument(Boolean Sending,
			long Check_Time, String To_Adress, String Cord, String Group_Name,
			int Key) {
		BasicDBObject doc = new BasicDBObject();
		doc.put("sending", Sending);
