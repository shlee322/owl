...
public class Sender {
	...
	public static void main(String ar[])
	{
		...
		for(int i=0; i<50; i++) //차후 IP를 받으면 50(IP수)*16개 = 800, 물론 처리구조를 약간바꿔서
			new TaskThread().start();
		
		System.out.println("처리 스래드 준비 완료");
		
		System.out.print("시작:");
		System.out.println(System.currentTimeMillis());
		
		Task task = new Task(1, "test@laeradr.com", "테스트","테스트입니다.");
		try {
			for(int i=0; i<2; i++)
		...
		{//
			try {
				...
				{
					len = in.read(buffer);
					String string = new String(buffer, 0, len);//왠지 그냥 버퍼만 적으면 문제가 있을것 같음.
		...
		String []Data;
		 ...
		 try {
			...
			while ((line = reader.readLine()) != null) {
				 ...
				 Data = line.split(" "); //Data[3] : 우선순위, Data[7] : 메일서버
